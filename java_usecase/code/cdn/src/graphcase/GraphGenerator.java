...
public class GraphGenerator {
    ...
    public static Graph tree(int V) {
        ...
        //起点已经生成好了v个：邻接表的adj[0]－adj[v-1]个头结点
        //生成v-2个终点： 点[0，v-2)随机生成v-2个
        int[] prufer = new int[V-2];
        for (int i = 0; i < V-2; i++)
            prufer[i] = StdRandom.uniform(V);
//        System.out.println("随机生成V－2个终点" + Arrays.toString(prufer));
        
        // degree of vertex v = 1 + number of times it appers in Prufer sequence
        //假设v-1条边，起点为0、1、2、、、V－1.  终点为0、1、2
        int[] degree = new int[V];
        for (int v = 0; v < V; v++)
            degree[v] = 1;//初始化v个起点的度：出度全为1
        for (int i = 0; i < V-2; i++)
            degree[prufer[i]]++;//v-1个终点后的出度。每出现一个终点，这个点的入度都要加1
//        System.out.println("计算每个点的度" + Arrays.toString(degree) );
...
//        System.out.println("度为1只能用一次" + pq.min() );
        // repeatedly delMin() degree 1 vertex that has the minimum index
        //循环： 取出度为1的点，然后重新计算度，再加入度为1的点。
        for (int i = 0; i < V-2; i++) {
            int v = pq.delMin();
            G.addEdge(v, prufer[i]);
//            System.out.println("[取出度为1的起点，取生成的第i个终点]" + v+" , " + prufer[i]);
            degree[v]--;
            degree[prufer[i]]--;
//            System.out.println("重新计算每个点的度" +  Arrays.toString(degree)+"插入度为1的点" );
     ...
     * 2、在生成V－2个点，共有V－2个度
     */
    public static GraphNet minConGraph(int V) {
    	GraphNet G = new GraphNet(V);
